...
function check_product()
{
	...
	if (echo -n $1 | grep -q -e "^infinite_") ; then
		INFINITE_BUILD=$(echo -n $1 | sed -e 's/^infinite_//g')
		export BUILD_NUMBER=$( (date +%s%N ; echo $INFINITE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
	else
		INFINITE_BUILD=
	fi
	export INFINITE_BUILD
